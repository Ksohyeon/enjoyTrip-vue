<template>
    <div>
        ...
        <form class="d-flex my-3" onsubmit="return false;" role="search">
          ...
          <b-form-select v-model="selectedSido" :options="sidoOptions" value-field="value" text-field="text">
                <option value="0">검색 할 지역 선택</option>
        </b-form-select>
          <b-form-select v-model="selectedCategory" :options="categoryOptions" value-field="value" text-field="text">
      ...
      aria-label="검색어"
    ></b-input>
    <b-button id="btn-search" class="btn btn-outline-success" @click="search" type="button">
      검색
    </b-button>
        </form>
        <!-- kakao map start -->
   ...
   displayMarker(){
    ...
    for( var i = 0; i < this.searchResult.length; i++){
        positions.push({
            ...
            latlng: new window.kakao.maps.LatLng(this.searchResult[i].lat,this.searchResult[i].lon),
        });
